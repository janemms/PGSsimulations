...
```
Take 20th, 50th and 80th quantile from the distribution of environmental effects
-> 
```{r}
source("PGSvariance.R")
Ns = c(10000, 50000, 100000, 200000) # of individuals
...
qs = c(0.2, 0.5, 0.8) # quantiles of the environmental effects distribution
es = apply(as.matrix(qs), 2, function(x){qnorm(x, 0, sqrt(1 - h2))}) # environmental effects corresponding to quantiles qs of the environmental effect distribution
res = list()
res.idx = 1 # counter
...
for (e in es){
  ...
  # compute genetic effects for each pgs sample
  pgs.samples = scale(t(pgs.samples)) # scale each row to have mean = 0 and var = 1, (N x n.samples) matrix
  g = pgs.samples * sqrt(h2) # genetic effects, (N x n.samples) matrix of n.samples genetic effects for each individual
...
```
Proportion of individuals whose ranking is "uncertain", ie. their proportion of rankings over threshold is in (0.4, 0.6).
```{r}
# Extract the data frames for each N
res[[10]]
props = c()
for (f in res){
  props = c(props, f[1,5])
...
dim(props) = c(4,3)
colnames(props) = c("e = 0.2", "e = 0.5", "e = 0.8") # e = 0.2 means environmental component corresponds to the 20th percentile of the distribution of environmental effects
rownames(props) = c("N = 10 000", "N = 50 000", "N = 100 000", "N = 200 000")
props
...
df$qs <- as.factor(round(df$e, 4))

# assuming you collect results like:
# df <- data.frame(N = ..., qs = ..., prop.mid = ...)

ggplot(df, aes(x = N, y = prop.mid, color = qs, group = qs)) +
  geom_line() +
  geom_point() +
  scale_x_continuous(trans = "log10") +
  labs(
    title = "Mid-Risk Individuals vs Sample Size",
    x = "Sample Size (N)",
    y = "Proportion in 0.4–0.6 Risk Zone",
    color = "Environmental effect (e)"
  ) +
  theme_minimal()
...
### Disease prevalence in the top-PGS quantile
```{r}
