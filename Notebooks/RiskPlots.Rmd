...
- recommend resporting PGIs wirh confidence intervals, and interpreting them with caution


**Ware, E. et al** Heterogeneity in polygenic scores for common human traits

- Examined the impact of using genotyped vs imputed SNPs, different p-value thresholds, and different LD trimming methods on the predictive power of PGSs and the correlation among scores.

"the sample size for these GWAS needs to be upwards of 10,000 cases and controls for diseases controlled by 1000 loci with a mean relative risk of 1.04"

**Sun, J. et al** Translating polygenic risk scores for clinical use by estimating the confidence bounds of risk prediction

- Introduce MCCP, a conformal prediction method that provides individual-level confidence estimates for polygenic risk predictions.
- It splits data into training and calibration sets to compute class-specific p-values for case and control labels.
- Unlike standard PRS percentiles, MCCP guarantees a bounded error rate and can withhold predictions if confidence is low.
- Repeated cross-validation ensures stable, calibrated predictions with explicit uncertainty regions for each individual.

--> Provides absolute confidence of classification per individual

- Unlike standard PRS percentiles that compare individuals to the population, MCCP provides calibrated, individual-specific uncertainty bounds, ensuring predictions are only made when confidence meets a preset threshold.

**Schultz, L. M. et al** Stability of polygenic scores across discovery genome-wide association studies
...
```
CHD PGS000349: h2 = 0.5, K = 0.05, N.case = 60 000, N.ctrl = 120 000, M = 70, n = 4800, samples

CHD PGS000019: h2 = 0.5, K = 0.05, N.case = 60 000, N.ctrl = 120 000, M = 192, n = 725, samples

CHD PGS000200: h2 = 0.5, K = 0.05, N.case = 15 000, N.ctrl = 15 000, M = 28, n = 725, samples

Lung cancer: h2 = 0.2, K = 0.05, N.case = 60 000, N.ctrl = 120 000, M = 16, n = 30 000
...
#ggsave(filename = paste0("PGSrisk_CHD_0019", ".png"), 
#       plot = last_plot(), 
#       width = 8, height = 5, dpi = 300, bg = "white")
...
Simulaatioissa näyttää, että epävarmuus persentiilin sairastumisriskissä kasvaa SNP-määrän kasvaessa, mutta uskoisin että todellisuudessa suurempi SNP-määrä kuvaa geneettistä riskiä paremmin.
