...
2) Studied the proportion of certain-above -threshold inividuals among those whose PGS point estimate exceeds the threshold, as a function of GWAS sample size and heritability.

We have studied the variability in individuals ranks by:
1) Sampling PGS estimates for individuals, and computing rankings for the individuals in each sample, we have studied the mean ranking of an individual, as well as the range of rankings.

2) For each individual we have studied the proportion of rankings in the top-PGS quantile

In relation to the liability threshold model of disease susceptibility, we have:
...
PGS.uncertainty <- function(N, M, n, h2, S = -0.75, n.samples = 1000){
  ...
  sigma.est = 1/N *(1 - (var*2*ps*(1-ps))) # effects of SNPs can be estimated with variance proportional to their allele frequency (rare alleles have larger effect sizes, and larger effect sizes can be estimated with smaller uncertainty), scaled
  ...
  pgs.var = rowSums((X.scaled^2) * sigma.est) # PGS variance calculated analytically
...
Ks = c(0.1, 0.01, 0.001) # lifetime prevalences of disease
M = 1000 # of SNPs
h2s = c(0.2, 0.5, 0.8) # heritabilities
n = 1000 

risk.by.K = list()
percentile.summary = data.frame()
...
for (K in Ks) {
  ...
  cutpoints = quantile(g, probs = seq(0,1,0.01)) # divide into percentiles
  percentiles = cut(g, breaks = cutpoints, include.lowest = TRUE, labels = FALSE)
    
  for (d in 1:100) {
      idx = which(percentiles == d) # 
      mean.risk = mean(prop.over.t[idx])
      sd.risk = sd(prop.over.t[idx])
      percentile.summary = rbind(percentile.summary,
                      data.frame(K = K, h2 = h2, percentileile = d, mean.risk = mean.risk, sd.risk = sd.risk))
}
  }
...
for (K in Ks) {
  subset.K = percentile.summary[percentile.summary$K == K, ] # decile summary for current K
  ...
  for (i in seq_along(h2s)) {
    ...
    lines(subset.h2$percentile, subset.h2$mean.risk, type = "b", 
...
When the effect sizes are sampled and PGS rankings collected several times, we can observe how much the individual rankings change due to the effect estimation uncertainty. We collect
...
pgs.sample.rankings = PGS.sample.rankings(N, M, n, h2, n.samples, S = -0.75, t)
...
```
When varying sample size N and heritability, the mean range of rankings decreases with increasing sample size and heritability. 
```{r}
# consider only N = 500000 and h2 = 0.5
rankings.Nlarge = ranking.stats[,c(1, 3, 4)]
...
M = 1000 # of SNPs
h2 = 0.5 # heritability
N = 50000 # GWAS sample size
n.samples = 100 # of samples calculated for each individual
t = 0.9 # 
n = 10000 # target sample size

v.rankings = PGS.sample.rankings(N, M, n, h2, n.samples, S = -0.75, t) # simulate data
...
combined_plot = plot1 / plot2 # compine plots one below other
combined_plot
```
...
pgs.ranks.scaled = (pgs.ranks) / n # scale rankings to [0,1]
...
# print first indices
cat("Below threshold:", head(below_inds), "\n")
cat("Above threshold:", head(above_inds), "\n")
cat("Overlaps threshold, mean below:", head(touch_below_inds), "\n")
cat("Overlaps threshold, mean above:", head(touch_above_inds), "\n")

```

Visualize the ranking distribution of a selected individual, with confidence intervals and classification threshold shown, as well as the corresponding desease risks as a boxplot.
```{r}
library(ggplot2)
...
We want to model the true polygenic risk in population. To do this we first generate the phenotypes and true genetic effects (betas). Using the true betas, we compute the polygenic score for a large number of individuals, yielding us the true genetic values of said individuals. We use large enough N to be able to precisely estimate the genetic values at the percentiles of the PGS distribution. 
...
To study this, I simulated the PGS samples for a set of individuals and computed disease risks for different disease prevalences (GWAS N = 500 000, PGS n = 10 000, h² = 0.5, K = 0.001, 0.01, 0.1). For each individual, 1000 PGS estimates were calculated, and the corresponding disease risks were compared to the true disease risk (computed using the true betas). 

The disease risks were studied further for two sets of individuals:
- 10 individuals with most variation in the PGS estimates
- 10 individuals with most variation in the disease risks computed using PGS estimates

From the boxplots of disease risk we see the variation in the disease risk for said individuals. The red dot indicates the true disease risk of the individual. 
...
M = 1000 # of SNPs
h2 = 0.5 # heritability
n.samples = 1000 # of samples for each individual
Ks = c(0.001, 0.01, 0.1) # disease prevalences

pgs = PGS.uncertainty(N, n, M, h2) # simulate data
...
```
The individuals in the first plot, with largest variation in the PGS estimates, the variation in the disease risk is very variable. This indicates that high variation in the PGS estimates does not necessarily cause large variation in the disease risk. For example, if the individual ranks far away from the liability threshold by their genetic component, the variation in PGS estimates does not cause variation in the disease risk estimates.

In the second plot, the individuals whose disease risk varies higly, all have high risk of disease. These individuals rank close to the liability threshold by their genetic component, and thus even small changes in PGS estimates cause large variation in the disease risk. 

Either way, there is large variation in disease risk for the individuals in both sets. 

The third plot presents the diesease risk as a function of PGS, with the two sets of individuals marked in the figure.
```{r}
...
```
The individuals with largest variation in PGS estimates lie evenly around the curve, but the individuals with largest variation in disease risk lie on the steepest part of the curve.

The figures illustrate the variability in individual's PGS estimates and disease risks. In these figures the disease risk is computed directly from the PGS estimate, and thus the relative ordering of individuals in the simulation does not affect the risk estimates.
...
PGS.risk.by.ranking <- function(N, M, n, h2, K, n.samples, S = -1){
  ...
  # compute PGS estimates
  PGS = PGS.sample.rankings(N, M, n, h2) # simulate data
  X = PGS$geno # get genotypes
  beta.samples = PGS$beta.samples # estimated genetic effects
  
  pgs.est = X %*% t(beta.samples) # PGS samples (n x n.samples)
  
  pgs.sc = scale(pgs.est) # scale pgs estimates
  ...
  return(list(
    pgs = pgs.mean, # a vector of PGS point estimates for n individuals
    pgs.est = pgs.est, # a (n x n.samples) matrix of PGS samples
    risks = risks, # a (n x n.samples) matrix of disease risks for n individuals
    min.risks = min.risks, 
    max.risks = max.risks, 
...
With the target sample size large (n = 1000 or 10 000), uncertainty in disease risk decreases with increasing prevalence K. With a small target sample (n = 100), the uncertainty is largest when k = 0.01. This is explained by the small sample size: when n = 100, with prevalence 0.1 there are ~ 10 diseased individuals, and small changes in the number of diseased individuals do not cause large changes in the disease risk. With prevalence 0.001 there are likely no diseased individuals, and thus the disease risk does not vary that much. With prevalence 0.01, however, there are likely 0 or 1 diseased individuals, wich causes the risk of disease to vary greatly. 
...
The uncertainty in the disease risk decreases with increasing number of cases, as when the effective sample size increases, the uncertainty in the GWAS effect sizes decreases. This way the uncertainty in individuals PGS estimates decreases, decreasing the uncertainty in disease risk.

As the heritability increases, the disease risk curve gets steeper with environment having a smaller inpact in the disease risk. In simulations there is no clear connection between heritability and disease risk. 

The target sample size n has a significant impact on the uncertainty in the disease risk. The uncertainty is the greatest with small n, but with n = 1000 and 10 000 the uncertainty decreases significantly.
...
voiko soveltaa käytäntöön ja onko lähestyminen sama kuin Ding et al paperissa?

Ding et al: 
- jokaiselle yksilölle lasketaan GV posteriorijakauma, joka avulla lasketaan posteriori sd sekä credible intervallit PRSlle. 
- Paperissa johdetaan myös analyyttinen laskukaava yksilön PRS sd:lle. 
