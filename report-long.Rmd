...
PGS.sample.rankings <- function(N, M, n, h2, n.samples = 1000, S = 0.75, t = 0.9){
  ...
  var = h2/((2*ps*(1-ps))^S*M) # variance proportional to negative selection coefficient, scaled to match heritability h2 and M SNPs
  sigma.est = pmax(1/N * (1 - var), 0) # effects of SNPs can be estimated with variance proportional to their allele frequency (rare alleles have larger effect sizes, and larger effect sizes can be estimated with smaller uncertainty)
...
pgs.ind <- pgs.est[individual, ]  # individual's PGS samples
